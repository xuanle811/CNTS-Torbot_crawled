### KHI NAO CRAWL CHECKOUT THI rename file as linktree.py
...
class LinkTree(Tree):
    def __init__(self, url: str, depth: int, client: httpx.Client, cookies: dict = None) -> None:
        super().__init__()
        self._url = url
        self._depth = depth
        self._client = client
        self._cookies = cookies  # Lưu cookies vào đây
    ...
    def _append_node(self, id: str, parent_id: str or None) -> None:
        ...
        """
        try:
            # Thêm self._cookies vào đây
            resp = self._client.get(id, cookies=self._cookies) 
    ...
    def _build_tree(self, url: str, depth: int) -> None:
        ...
        if depth > 0:
            ...
            # Thêm self._cookies vào lệnh get này
            resp = self._client.get(url, cookies=self._cookies)
